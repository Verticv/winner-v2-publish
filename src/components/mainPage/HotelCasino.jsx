...
const HotelCasino = () => {
  ...
  const Card1 = ({ img, imgHover, text, disabled }) => {
    ...
    return (
      <div style={{ height: '207px', width: '411px' }} className="flex items-end relative cursor-pointer justify-center" onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
        ...
        <p style={{ color: '#ffdfbd', marginBottom: '129px' }} className='z-20 absolute -bottom-55px left-32px text-16px tracking-tighter font-spoqa h-16px flex items-center'>{text}</p>
      </div>
    )
  }
