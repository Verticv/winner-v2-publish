...
const Tabs = () => {
  ...
  const SingleTab = ({ title, icon }) => (
    <button style={{width:'115px', height:'109px'}} className='relative flex flex-col items-center justify-between pb-24px hover:opacity-90'>
      <img src={icon} alt="icon1" className='object-none z-20' style={{marginTop:'17px'}} />
      <h2 style={{color:'#252525', fontSize:'14px'}} className='tracking-tighter z-20 font-spoqaBold'>{title}</h2>
      <img src={TabBg} alt="bg" className='object-none absolute' />
    </button>
  )
  return (
    <div style={{width:'1260px'}} className="flex justify-center items-center">
      <img src={Left} className="object-none -mr-3px mb-5px" alt="" />
      ...
      <img src={Right} className="object-none -ml-3px mb-5px" alt="" />
    </div>
  )
}
