...
const MinigameBetHistoryPowerballTable = ({array, checkedState, setCheckedState, isPopupOpen, setPopupOpen}) => {
    ...
    function Cells({ items }) {
        ...
        const detailButton = (
            <button 
                style={{height:"29px", width: "73px", backgroundColor:"#826140"}} 
        ...
        return items.map(item => (
            <div className="flex items-center bg-gray-fefefe font-spoqa text-14px tracking-tight text-gray-r585858 h-56px w-full border-b border-gray-dddddd flex-shrink-0">
                ...
                <div style={{width: "123px", color: item.profit.includes("+") ? "#e65454" : "#585858"}} className="font-robotoRegular text-right">{item.profit}</div>
                <div style={{width: "80px", color: item.status === "승" ? "#d52e2e" : "#585858"}} className="text-center">{item.status}</div>
